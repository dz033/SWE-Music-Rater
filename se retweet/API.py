...
@contract(Schema({'id': int}))
def repost(request):
    flake_id = request.payload.get('id')
    flake = service.flake.get(flake_id)
    if flake is None:
        return client_error('INVALID', f"No flake exists with id: {flake_id}")
    repost = request.user.repost(flake)
...
@post("repost/delete")
@contract(Schema({'id': int}))
def delete_repost(request):
    repost_id = request.payload.get('id')
    repost = service.flake.get_repost(repost_id)
    if repost is None:
        return client_error('INVALID', f'No repost with id: {repost_id}')
    if repost.user != request.user:
        return client_error('PERMISSION_ERROR', 'You do not have permission to delete this repost')
    request.user.delete_repost(repost)
    return success({'id': repost_id})