...
class User(models.Model):
    ...
    #added retweet flake function that takes in the target flake and updates parameters in the retweet 
    def retweet_flake(self, flake):
        return Flake.objects.create(
            author = flake.author,
            content = flake.content,
            image = flake.image,
            reply_to = flake.reply_to,
            is_retweet = True,
            retweet_user = self,
            original_post_date = flake.creation_date
        )
    ...
    def list_flakes(self):   #CHANGED THIS FUNCTION TO INCLUDE RETWEETS
        return Flake.objects.filter((Q(author=self) or Q(retweet_user=self)) & Q(reply_to__isnull=True)).order_by("-creation_date")
